...
func test_get_setting_returns_default_when_not_set() -> void:
	...
	var result: bool = CodotSettingsRef.get_setting("enable_prompt_panel", false)
...
func test_setting_definitions_has_required_keys() -> void:
	for key: String in CodotSettingsRef.SETTING_DEFINITIONS:
		var def: Dictionary = CodotSettingsRef.SETTING_DEFINITIONS[key]
...
func test_is_feature_enabled_returns_bool() -> void:
	var result: bool = CodotSettingsRef.is_feature_enabled("prompt_panel")
	assert_true(result is bool, "Should return a boolean")


func test_is_feature_enabled_handles_unknown_feature() -> void:
	var result: bool = CodotSettingsRef.is_feature_enabled("nonexistent_feature")
...
func test_known_features_exist() -> void:
	...
	for feature in features:
		var result: bool = CodotSettingsRef.is_feature_enabled(feature)
...
func test_settings_prefix_is_correct() -> void:
	assert_eq(CodotSettingsRef.SETTINGS_PREFIX, "plugin/codot/", 
		"Prefix should be 'plugin/codot/'")


func test_get_all_settings_returns_dict() -> void:
	var all_settings: Dictionary = CodotSettingsRef.get_all_settings()
...
func test_has_port_settings() -> void:
	assert_true(CodotSettingsRef.SETTING_DEFINITIONS.has("mcp_server_port"),
		"Should have MCP server port setting")
	assert_true(CodotSettingsRef.SETTING_DEFINITIONS.has("vscode_port"),
		"Should have VS Code port setting")


func test_port_defaults_are_valid() -> void:
	var mcp_port: int = CodotSettingsRef.SETTING_DEFINITIONS["mcp_server_port"].default
	var vscode_port: int = CodotSettingsRef.SETTING_DEFINITIONS["vscode_port"].default
...
func test_has_enable_settings() -> void:
	...
	for setting in expected_enables:
		assert_true(CodotSettingsRef.SETTING_DEFINITIONS.has(setting),
			"Should have '%s' setting" % setting)


func test_has_auto_archive_setting() -> void:
	assert_true(CodotSettingsRef.SETTING_DEFINITIONS.has("auto_archive_on_send"),
		"Should have auto_archive_on_send setting")
	assert_eq(CodotSettingsRef.SETTING_DEFINITIONS["auto_archive_on_send"].default, true,
